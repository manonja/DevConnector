...
router.post(
  ...
  async (req, res) => {
    ...
    // Turn skills into an array and handle users white spaces
    if (skills) {
    ...
    try {
      // Find the profile thanks to the token 
...
router.put('/education', [auth, 
  [
    check('school', 'School is required')
      .not()
      .isEmpty(),
    check('degree', 'Degree are required')
      .not()
      .isEmpty(),
    check('field', 'Field of study date are required')
    .not()
    .isEmpty()
]], async (req, res) => {
  const errors = validationResult(req)
...
router.delete('/experience/:ed_id', auth, async (req, res) => {
  try {
    const profile = await Profile.findOne({ user: req.user.id})
    // Get remove index
